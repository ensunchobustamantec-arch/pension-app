...
export default function Pagos() {
  ...
  async function load() {
    const [{ data: pags }, { data: ests }] = await Promise.all([
      supabase.from('pagos').select('*, estudiantes(id, tarifa, profiles(nombre))').order('created_at', { ascending: false }),
      supabase.from('estudiantes').select('id, tarifa, profiles(nombre)').eq('activo', true)
    ])
    setPagos(pags || [])
    setEstudiantes(ests || [])
    setLoading(false)
